<template>
  <div class="container">
    <input type="checkbox" class="checkbox" v-on:change="toggleUser"/>
    <p v-if="!isEditing" v-bind:class="{'is-status':user.status}">{{user.voornaam}}</p>
    <form v-else @submit.prevent="endEditing()" class="form-control">
...
export default {
  ...
  methods: {
    ...
    startEditing() {
      if(!this.isEditing) {
        this.newUser = this.user.voornaam;
        this.isEditing = true;
      }
      else {
          this.endEditing();
